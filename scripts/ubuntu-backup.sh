...
acquire_lock() {
  ...
  echo "Another LayerPilot backup/restore operation appears to be running (lock: $LOCK_DIR, pid: $owner)." >&2
...
prune_backups() {
  ...
  echo "Pruning LayerPilot backups older than $RETENTION_DAYS day(s) from $BACKUP_DIR..."
