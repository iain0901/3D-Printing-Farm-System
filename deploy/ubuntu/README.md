# LayerPilot Ubuntu Deployment

This guide deploys LayerPilot on Ubuntu 22.04 or 24.04 with Docker Compose. The app runs on port `8797`; Compose binds that port to `127.0.0.1` by default so Nginx can proxy public HTTP/HTTPS traffic to it.
...
If an existing `/etc/docker/daemon.json` differs from LayerPilot's sample, `install-log-rotation` stops and asks you to merge settings manually. Set `LAYERPILOT_OVERWRITE_DOCKER_DAEMON=true` only when you intentionally want the helper to back up and replace that file.
