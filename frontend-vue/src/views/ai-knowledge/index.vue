...
<script>
  ...
  export default {
    name: 'AiKnowledge',
    data() { return { loading: false, items: [], dialogVisible: false, editingId: '', saving: false, form: blank() } },
    created() { this.load() },
    methods: {
      async load() { this.loading = true; try { this.items = await fetchAiKnowledge() } finally { this.loading = false } },
      openCreate() { this.editingId = ''; this.form = blank(); this.dialogVisible = true },
      openEdit(item) { this.editingId = item.id; this.form = { title: item.title, category: item.category, tagsText: (item.tags || []).join(', '), content: item.content, enabled: item.enabled !== false }; this.dialogVisible = true },
      resetForm() { this.form = blank(); this.editingId = '' },
      async save() {
        if (!this.form.title || !this.form.content) return this.$baseMessage('請填寫標題與可引用內容。', 'warning')
        this.saving = true
        try {
          const payload = { title: this.form.title, category: this.form.category || 'general', content: this.form.content, tags: this.form.tagsText.split(',').map((tag) => tag.trim()).filter(Boolean), enabled: this.form.enabled }
          ...
          if (index === -1) this.items.unshift(item); else this.items.splice(index, 1, item)
          this.dialogVisible = false
          this.$baseMessage('AI 知識條目已儲存。', 'success')
        } finally { this.saving = false }
      },
      async remove(item) {
        try { await this.$confirm(`刪除「${item.title}」後，Chatwoot AI 不會再引用此內容。`, '確認刪除', { type: 'warning' }); await deleteAiKnowledge(item.id); this.items = this.items.filter((entry) => entry.id !== item.id); this.$baseMessage('已刪除知識條目。', 'success') } catch (_) {}
      }
    }
  }
</script>
...
<style lang="scss" scoped>
  .ai-knowledge-page { padding: 20px; } .toolbar { display: flex; gap: 10px; margin-bottom: 16px; } .tag { margin: 2px 4px 2px 0; }
</style>
