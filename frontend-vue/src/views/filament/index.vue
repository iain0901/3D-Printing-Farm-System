  ...
  <div class="filament-container">
    <el-tabs v-model="tab">
      <el-tab-pane label="線材庫存" name="spools">
        <div class="quickbar">
          <el-button v-permissions="['inventory:write']" type="primary" icon="CirclePlus" @click="spoolDialogVisible = true">
            新增線材
          </el-button>
        </div>
        <el-table :data="spools" style="width: 100%">
          ...
          <el-table-column v-permissions="['inventory:write']" label="操作" width="140">
            <template #default="{ row }">
      ...
      <el-tab-pane label="採購申請" name="purchases">
        <div class="quickbar">
          <el-button v-permissions="['inventory:write']" type="primary" icon="CirclePlus" @click="purchaseDialogVisible = true">
            新增採購申請
          </el-button>
        </div>
        <el-table :data="purchaseRequests" style="width: 100%">
          ...
          <el-table-column v-permissions="['inventory:write']" label="操作" width="120">
            <template #default="{ row }">
              <el-button size="small" :disabled="row.status === 'received'" :loading="receiveBusy === row.id" @click="receive(row)">
                已收貨
              </el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="spoolDialogVisible" title="新增線材" width="420px">
      <el-form label-width="80px" size="small">
        ...
        <el-form-item label="總重 g">
          <el-input-number v-model="spoolForm.weight" :min="1" style="width: 100%" controls-position="right" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div>
    ...
    <el-dialog v-model="purchaseDialogVisible" title="新增採購申請" width="420px">
      <el-form label-width="80px" size="small">
        ...
        <el-form-item label="數量">
          <el-input-number v-model="purchaseForm.quantity" :min="1" style="width: 100%" controls-position="right" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div>
