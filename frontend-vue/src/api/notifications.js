...
export function createNotificationChannel(payload) {
  ...
  return request({ url: '/api/notificationChannels', method: 'post', data: payload, headers: idempotencyHeaders(key, payload) }).then(
    (r) => {
...
export function updateNotificationChannel(id, patch) {
  ...
  return request({
    url: `/api/notificationChannels/${id}`,
    method: 'patch',
    data: patch,
    headers: idempotencyHeaders(key, { id, patch }),
  }).then((r) => {
    clearIdempotency(key)
    return r
  })
