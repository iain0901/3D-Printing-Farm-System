...
export function autoSchedule() {
  ...
  return request({ url: '/api/schedule/auto', method: 'post', data: BASE_OPTIONS, headers: idempotencyHeaders(key, BASE_OPTIONS) }).then(
    (result) => {
...
export function optimizeSchedule(strategy) {
  ...
  return request({ url: '/api/schedule/optimize', method: 'post', data: payload, headers: idempotencyHeaders(key, payload) }).then(
    (result) => {
      clearIdempotency(key)
      return result
    }
...
export function solveConstraintSchedule(objective) {
  ...
  return request({ url: '/api/schedule/constraint', method: 'post', data: payload, headers: idempotencyHeaders(key, payload) }).then(
    (result) => {
      clearIdempotency(key)
      return result
    }
