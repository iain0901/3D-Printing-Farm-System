...
- Cost catalogs are scoped per workspace. A signup-created tenant can change machine and material rates without changing another workspace's quote calculator, file estimate, generated sample, upload metadata, parametric model, or slicer estimate results; scoped state and admin exports omit the raw multi-tenant `costCatalogs` map.
