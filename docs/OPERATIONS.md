...
- Enabling TOTP requires the current account password plus the authenticator code. Review `auth.2fa_enable_failed` events when investigating suspicious enrollment attempts; these records include workspace/operator context and the failure reason without submitted passwords, TOTP secrets, or codes.
