...
Stripe billing webhooks on `/api/billing/webhook/stripe` verify the official `Stripe-Signature` header against `LAYERPILOT_STRIPE_WEBHOOK_SECRET` when Stripe calls the app directly. Deployments that terminate or transform webhook bodies at a trusted edge proxy can instead inject `x-layerpilot-billing-webhook-secret` with the same configured secret; do not expose that fallback header to arbitrary clients. Stripe `event.id` values are stored in a compact internal replay ledger so duplicate provider deliveries return `x-layerpilot-stripe-webhook-replay: true` and do not create another `billing.stripe_webhook` audit event.
...
In production, send operational shared tokens only in headers. Metrics scrapers must use `x-layerpilot-metrics-token`, and worker-to-API broadcasts must use `x-layerpilot-worker-token`; token query parameters are accepted only outside production for local compatibility so secrets do not land in proxy or access-log URLs. User session and API-key credentials are also rejected from URL query parameters in production. Browser realtime clients request a short-lived `/api/events/token` ticket with the normal `Authorization` header, then use that one-time ticket for WebSocket or SSE connection URLs.

There is no self-service signup route: an exposed customer VPS cannot mint arbitrary new Owner workspaces. Each customer gets its own independently provisioned deployment created with `scripts/provision-tenant.sh`, which generates a private `.env` with a unique Compose project/container/port and a bootstrap Owner account; additional operators are then added from the Team page by that Owner.
...
- Enabling TOTP requires the current account password plus the authenticator code. Review `auth.2fa_enable_failed` events when investigating suspicious enrollment attempts; these records include workspace/operator context and the failure reason for invalid password or authenticator-code attempts without submitted passwords, TOTP secrets, or codes.
...
- Review `/api/audit` for successful login, logout, password-change, and 2FA setup/enable/verify/disable events during access reviews or incident response. These events include workspace, user, actor, and session metadata where applicable, but do not store bearer tokens, passwords, TOTP secrets, or recovery codes.
...
- Review `/api/audit` after Stripe smoke tests to confirm each Stripe provider event ID creates only one `billing.stripe_webhook` event; provider retries should be visible through the replay response header rather than duplicate audit evidence.
...
Committed restores add `admin.restore_prepared` evidence to the restored audit timeline. Review it after restore drills to confirm the operator, workspace, collection counts, warning count, stripped-storage-path count, restored file-payload count, and compact file-payload coverage counts without exposing restored record names, user/customer emails from the backup, storage paths, file bytes, or the backup body.
