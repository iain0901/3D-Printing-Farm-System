...
- [ ] User session/API-key credentials are not accepted in production URL query parameters; browser realtime WebSocket/SSE clients use short-lived one-time `/api/events/token` tickets instead of long-lived bearer tokens in connection URLs.
...
- [ ] Admin and Owner users enroll TOTP when `requireAdmin2fa` is enabled; in `NODE_ENV=production`, unenrolled Owner/Admin sessions are blocked from protected APIs until enrollment, TOTP enablement requires the current account password plus authenticator code, and enrolled Owner/Admin users cannot disable TOTP while the workspace policy remains enabled.
...
- [ ] `/api/audit` shows failed TOTP enablement password-proof and invalid-code attempts as `auth.2fa_enable_failed` with workspace/operator context and no submitted passwords, TOTP secrets, or authenticator codes.
...
- [ ] `/api/audit` shows printer setup, generated-file, and real printer-action events with workspace/operator context for `printer.created`, `printer.updated`, `printer.action`, `file_folder.created`, `file_folder.reused`, `file.sample_generated`, `hot_drop.handled`, and `parametric.generated`, without generated STL bodies, storage paths, object-storage keys, bridge endpoint URLs, API keys, or printer credentials.
...
- [ ] Signup-created tenant workspaces have isolated cost catalogs: `/api/state` and `/api/admin/export` expose only the authenticated workspace's pricing catalog, and quote/file/slicer estimates use that workspace's rates rather than another tenant's overrides.
...
- [ ] Real printer action audit records have been reviewed after a bridge-connected smoke run to confirm `printer.action` includes the expected operator, printer, action, job, and bridge identifiers without bridge endpoint URLs or API keys.
...
- [ ] Direct file creation plus model/G-code download, preview, and deletion access are reviewed through `file.created`, `file.downloaded`, `file.previewed`, and `file.deleted` audit events; events include file identity and storage-backed context without storing file contents, local storage paths, object-storage keys, or raw reference records.
...
- [ ] Stripe billing webhooks have been smoke-tested with a valid `Stripe-Signature` header, or the route is reachable only through a trusted edge proxy that injects `x-layerpilot-billing-webhook-secret`; replay the same provider event once and confirm the response includes `x-layerpilot-stripe-webhook-replay: true` without creating a duplicate `billing.stripe_webhook` audit event.
