...
- [ ] `/api/audit` shows recent production/admin events with the expected workspace and operator context, including scheduling, queue, bridge, file-version, history/reprint, onboarding, support snapshot, settings, billing, add-on, cost catalog, API-key, and user-management changes.
