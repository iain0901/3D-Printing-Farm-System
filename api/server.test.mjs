...
describe("3DSTUXXX API", () => {
  ...
  it("publishes matching production events to MQTT and hides MQTT credentials", async () => {
    ...
    await withApp(async ({ app, dbPath }) => {
      ...
      expect(payload).toMatchObject({ service: "3DSTUXXX", event: { type: "queue.created", message: "MQTT published job.gcode queued" } });
...
endsolid s3_store`;
  ...
  it("runs backend slicer jobs, stores G-code output, and updates file metadata", async () => {
    await withApp(async ({ app, dbPath }) => {
      ...
      expect(gcode).toContain("Generated by 3DSTUXXX internal slicer adapter");
  ...
  it("persists inventory, maintenance, and order operations", async () => {
    await withApp(async ({ app, dbPath }) => {
      ...
      expect(labels.json().html).toContain("3DSTUXXX spool labels");
  ...
  it("builds analytics, print history, reprints completed jobs, and exports backups", async () => {
    await withApp(async ({ app, dbPath }) => {
      ...
      expect(exported.json()).toMatchObject({ service: "3DSTUXXX", analytics: expect.any(Object), history: expect.any(Array) });
